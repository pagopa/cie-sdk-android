package it.pagopa.io.app.cie.pace

import androidx.annotation.CheckResult
import it.pagopa.io.app.cie.CieLogger
import it.pagopa.io.app.cie.pace.utils.toFixedLengthUnsigned
import org.bouncycastle.jce.provider.BouncyCastleProvider
import java.math.BigInteger
import java.security.KeyPair
import java.security.KeyPairGenerator
import java.security.PublicKey
import java.security.interfaces.ECPublicKey
import java.security.spec.AlgorithmParameterSpec
import java.security.spec.ECGenParameterSpec
import javax.crypto.interfaces.DHPublicKey
import javax.crypto.spec.DHParameterSpec

internal class MappingKey {
    @CheckResult
    fun toRawData(pubKey: PublicKey): ByteArray? {
        return when (pubKey) {
            is ECPublicKey -> {
                val w = pubKey.w
                val fieldSize = (pubKey.params.curve.field.fieldSize + 7) / 8
                val xBytes = toFixedLengthUnsigned(w.affineX.toByteArray(), fieldSize)
                val yBytes = toFixedLengthUnsigned(w.affineY.toByteArray(), fieldSize)
                byteArrayOf(0x04) + xBytes + yBytes
            }

            is DHPublicKey -> dhPublicKeyRaw(pubKey)
            else -> null
        }
    }

    private fun dhPublicKeyRaw(dhKey: DHPublicKey): ByteArray {
        CieLogger.i("PUB KEY", "PARSING DH: bit length: ${dhKey.params.p.bitLength()}")
        val pLen = (dhKey.params.p.bitLength() + 7) / 8 // per 2048 bit = 256
        CieLogger.i("Expected p length", "$pLen bytes")
        val yBytes = dhKey.y.toByteArray()

        val unsigned = if (yBytes.isNotEmpty() && yBytes[0] == 0.toByte()) {
            yBytes.copyOfRange(1, yBytes.size) // tolgo byte di segno
        } else {
            yBytes
        }

        val back = if (unsigned.size < pLen) {
            ByteArray(pLen - unsigned.size) + unsigned // pad a sinistra
        } else if (unsigned.size > pLen) {
            unsigned.copyOfRange(unsigned.size - pLen, unsigned.size) // tronco se eccede
        } else {
            unsigned
        }
        CieLogger.i("Actual cie key length", "${back.size} bytes")
        return back
    }


    @Throws(Exception::class)
    @CheckResult
    fun createMappingKey(
        keyAgreement: KeyAgreementAlgorithm,
        parameterSpec: PACEDomainParam
    ): KeyPair {
        return when (keyAgreement) {
            KeyAgreementAlgorithm.DH -> {
                CieLogger.i("MappingKey", "DH\nparameterSpec:${parameterSpec.name}")
                val dhParams: DHParameterSpec = when (parameterSpec) {
                    PACEDomainParam.PARAM_ID_GFP_1024_160 -> RFC5114.dh_1024_160()
                    PACEDomainParam.PARAM_ID_GFP_2048_224 -> RFC5114.dh_2048_224()
                    PACEDomainParam.PARAM_ID_GFP_2048_256 -> RFC5114.dh_2048_256()
                    else -> throw IllegalArgumentException("Invalid DH parameter ID for PACE")
                }

                val keyGen = KeyPairGenerator.getInstance("DH", BouncyCastleProvider.PROVIDER_NAME)
                keyGen.initialize(dhParams)
                keyGen.generateKeyPair()
            }

            KeyAgreementAlgorithm.ECDH -> {
                CieLogger.i("MappingKey", "ECDH\nparameterSpec:${parameterSpec.name}")
                val curveName = parameterSpec.toCurveName()
                val ecSpec: AlgorithmParameterSpec = ECGenParameterSpec(curveName)

                val keyGen =
                    KeyPairGenerator.getInstance("ECDH", BouncyCastleProvider.PROVIDER_NAME)
                keyGen.initialize(ecSpec)
                keyGen.generateKeyPair()
            }
        }
    }
}


object RFC5114 {

    fun dh_1024_160(): DHParameterSpec {
        val p = BigInteger(
            "",
            16
        )
        val g = BigInteger(
            "",
            16
        )
        val q = BigInteger("F518AA8781A8DF278ABA4E7D64B7CB9D49462353", 16)
        return DHParameterSpec(p, g, q.bitLength())
    }

    fun dh_2048_224(): DHParameterSpec {
        val p = BigInteger(
            "",
            16
        )
        val g = BigInteger(
            "",
            16
        )
        val q = BigInteger("801C0D34C58D93FE997177101F80535A4738CEBCBF389A99B36371EB", 16)
        return DHParameterSpec(p, g, q.bitLength())
    }

    fun dh_2048_256(): DHParameterSpec {
        val p = BigInteger(
            "",
            16
        )
        val g = BigInteger(
            "",
            16
        )
        val q = BigInteger("8CF83642A709A097B447997640129DA299B1A47D1EB3750BA308B0FE64F5FBD3", 16)
        return DHParameterSpec(p, g, q.bitLength())
    }
}